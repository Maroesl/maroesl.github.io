<template>
  <li @click="ovoshow(item)">
    ...
    <div class="songs-list-info">
      ...
      <div class="over">
        <span
          v-for="(ovo, index) in item.song ? item.song.artists : item.ar"
          :key="index"
          class="oper"
        >
          ...
          <span
            v-if="
              (item.song && item.song.artists
                ? item.song.artists.length
                : item.ar && item.ar.length
                ? item.ar.length
                : 0) >
              index + 1
            "
      ...
      <span class="album" v-else>-{{ item.song.album.name }}</span>
    </div>
    <div class="icon-play">
      <span
